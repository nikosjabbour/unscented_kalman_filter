...
class UKF:
    ...
    def __init__(self, n, m, a, b, k):
        # print(f'eisai oraios!!!!')
        self.n = n  # number of states
        self.m = m  # number of measured variables

        # Uncscented kalman filter parameters

        self.alpha = a
        self.beta = b
        self.kappa = k

        self.lambda_ = (self.n + self.kappa) * self.alpha**2 - self.n
        self.gamma = np.sqrt(self.n + self.lambda_)

        # Weight computation (These weights are used in sigma )
    ...
    def resetUKF(self, _Q, _R, x_0):
        ...
        self.mngm = md.ModelState(self.n, x_0)
    ...
    def setCovariances(self, _Q, _R):
        for i in range(self.n):
            for j in range(self.n):
                self.Q[i, j] = 0  # Q matrix has zero values in i != j (process noise)
                if i == j:
                    self.Q[i, j] = _Q

        for i in range(self.m):
            for j in range(self.m):
                self.R[i, j] = 0   # R matrix has zero values in i != j (measurement noise)
                if i == j:
                    self.R[i, j] = _R
    ...
    def y_UKF_calc(self):
        ...
        for k in range(2 * self.n + 1):
            # for i in range(self.m):
            xi = self.x_sigma[:, k]
            self.y_sigma[:, k] = self.mngm.model_output(xi)  # xi ** 2) / 20.0
    ...
    def state(self, w):
        ...
        for j in range(2 * self.n + 1):
            xp = self.x_sigma[:, j]
            self.x_sigma_f[:, j] = self.mngm.model_state(w[0, 0],
                                                   xp)  # 0.5*xp + 25.0*(xp/(1.0 + xp**2)) + 8.0*np.cos(1.2*(w[0, 0] - 1.0))
